...
fn auto_correlate(buffer: &[Complex64], sample_rate: usize) -> f64 {
    ...
    for i in 0..speech_buffer.len() {
        for j in 0..speech_buffer.len() - i {
            ...
            autocorrelations[i] += (sample1.re * sample2.re + sample1.im * sample2.im) / speech_power; // Normalize by speech power
        }
    }

    let peak_index = autocorrelations.iter().enumerate().max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap()).unwrap().0;
